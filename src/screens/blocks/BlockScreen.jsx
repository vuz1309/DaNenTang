...
import {
  getListBlocksRequest,
  unBlockRequest,
} from '../../api/modules/block.request';
...
const BlockScreen = ({route}) => {
  ...
  const UnBlock = async id => {
    try {
      ...
      const newBlockList = blockList.filter(user => user.id != id);
      setBlockList(newBlockList);
      ToastAndroid.show('Gỡ block thành công', ToastAndroid.SHORT);
    } catch (err) {
  ...
  return (
    <View style={{flex: 1, backgroundColor: Colors.white}}>
      <HeaderSearch title={'Danh sách chặn'} onBack={goBack} />
      ...
      <ScrollView
        ...
        {blockList.length > 0 ? (
          blockList.map(fr => (
            <BlockCard key={fr.id} fr={fr} UnBlock={() => UnBlock(fr.id)} />
          ))
        ) : (
